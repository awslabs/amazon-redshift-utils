...
while getopts h:p:U:d:s:e:w:t:  option; do
      case "${option}"
      in
...
with aqua_run as (select svl.query, q.starttime
     ...
     aqua_run_md5 as (select aq.query,
                             md5(listagg(qt.text) within group (order by sequence)) md5,
                             (max(w.total_exec_time / 1000000.0)) as rs_aqua_exec_time,
                             Row_number()
                             OVER (PARTITION BY md5(listagg(qt.text) within group (order by sequence)) ORDER BY aq.starttime desc) as r_num
                      from aqua_run aq
                               join stl_querytext qt using (query)
                               join stl_wlm_query w using (query)
                      group by aq.query, aq.starttime),
     rs_run as (select qt.query                                                                                                rs_query,
                       max(w.total_exec_time) / 1000000.0                                                                      total_exec_time,
                       md5(listagg(qt.text) within group (order by sequence))                                                  md5,
                       Row_number()
                       OVER (PARTITION BY md5(listagg(qt.text) within group (order by sequence)) ORDER BY q.starttime desc) as r_num,
                       q.starttime
     ...
     Rs_execution_time as (select rs_query, total_exec_time as rs_exec_time, md5, max(r_num) as r_num
                           from rs_run
                           group by rs_query, total_exec_time, md5, r_num),
     aqua_execution_time as (select aq.query,aq.rs_aqua_exec_time, md5, max(r_num) as r_num from aqua_run_md5 aq group by query, md5, r_num,rs_aqua_exec_time)
...
with aqua_run as (select svl.query, q.starttime
  ...
  aqua_total_exection as (select sum(rs_aqua_exec_time)  as aq_tot_exec_sc, aq.md5, aq.query
                             from aqua_execution_time aq
                             group by aq.query, aq.md5),
     
     RS_total_exection as (select sum(rs_exec_time) as tot_exec_sec, md5 as rs_md5 from Rs_execution_time  group by md5)
