...
FROM pg_catalog.pg_class AS srcobj
  INNER JOIN pg_catalog.pg_namespace AS srcnsp ON srcobj.relnamespace = srcnsp.oid
  INNER JOIN pg_catalog.pg_constraint AS con ON srcobj.oid = con.confrelid
  INNER JOIN pg_catalog.pg_class AS tgtobj ON tgtobj.oid = con.conrelid
  INNER JOIN pg_catalog.pg_namespace AS tgtnsp ON tgtobj.relnamespace = tgtnsp.oid
