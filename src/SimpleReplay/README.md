...
* Replay for Serverless Simple Replay maps all individual users from extract to a single admin user. See "Setting up Secrets Manager" section for detailed steps.

#### Authorizing Access to Redshift using AWS Secrets Manager 
The integration between Simple Replay and AWS Secrets Manager is required to enable access to Redshift serverless clusters ( https://aws.amazon.com/redshift/redshift-serverless/ ). It can also be used to execute workloads which rely on Redshift's native IDP integration with non-IAM identity providers ( https://docs.aws.amazon.com/redshift/latest/mgmt/redshift-iam-access-control-native-idp.html ) 
...
| secret_name                                 |Optional    | Name of the AWS Secret setup using AWS Secrets Manager. Required for Serverless.                                                                                                                                                                                                                                  | “”                                                                                                                                                                                                   |
...
* Limitations specific to Serverless:
  * For Serverless, Simple Replay currently only supports Replaying on Target Cluster. Simple Replay Extract for Serverless is in works and will be available in near future.
