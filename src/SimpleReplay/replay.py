...
def replay_worker(
    ...
    if connections_processed:
        logger.debug(
            f"Max connection offset for this process: {worker_stats['connection_diff_sec']:.3f} sec"
        )
...
def start_replay(
    ...
    if g_config.get("limit_concurrent_connections"):
        ...
        connection_semaphore = manager.Semaphore(
            g_config.get("limit_concurrent_connections")
        )
            ...
            for query in transaction.queries:
                if "copy " in query.text.lower() and "from 's3:" in query.text.lower():
                    ...
                    if from_text:
                        ...
                        try:
                            replacement_copy_location = replacements[
                                existing_copy_location
                            ][0]
                        except KeyError:
                            logger.info(
                                f"No COPY replacement found for {existing_copy_location}"
                            )
                            continue
                        ...
                        replacement_copy_iam_role = replacements[
                            existing_copy_location
                        ][1]
                        ...
                        for r in iam_replacements:
                            query.text = re.sub(
                                r[0], r[1], query.text, flags=re.IGNORECASE
                            )
    ...
    for connection_log in connection_logs:
        for transaction in connection_log.transactions:
            if connection_log.time_interval_between_queries == "all on":
                ...
                is_calculate_time_interval = (
                    str(transaction.time_interval).lower() == "true"
                )
            if is_calculate_time_interval:
                for index, sql in enumerate(transaction.queries[1:]):
                    prev_sql = transaction.queries[index]
...
def get_connection_credentials(
    username, database=None, max_attempts=10, skip_cache=False
):
    ...
    if g_is_serverless:
        if g_config["secret_name"] is not None:
            ...
            secret_name = get_secret(
                g_config["secret_name"], g_config["target_cluster_region"]
            )
            ...
            rs_client = client(
                "redshift", region_name=g_config.get("target_cluster_region", None)
            )
        ...
        rs_client = client(
            "redshift",
            region_name=g_config.get("target_cluster_region", None),
            **additional_args,
        )
        for attempt in range(1, max_attempts + 1):
            try:
...
def unload_system_table(
    ...
    credentials = get_connection_credentials(
        g_config["master_username"], max_attempts=3
    )
    conn = db_connect(
        default_interface,
        host=credentials["host"],
...
def validate_config(config):
    if not bool(
        re.fullmatch(g_cluster_endpoint_pattern, config["target_cluster_endpoint"])
    ):
        logger.error(
    ...
    config["filters"] = validate_and_normalize_filters(
...
def main():
    ...
    if bool(
        re.fullmatch(
            g_serverless_cluster_endpoint_pattern, g_config["target_cluster_endpoint"]
        )
    ):
        logger.info(
            f"Replaying on Redshift Serverless Cluster {g_config['target_cluster_endpoint']}"
        )
    ...
    id_hash = hashlib.sha1(g_replay_timestamp.isoformat().encode("UTF-8")).hexdigest()[
        :5
    ]
    if g_config.get("tag", "") != "":
    ...
    for connection in connection_logs:
        ...
        if (
            connection.transactions[0].queries[0].start_time
            and connection.transactions[0].queries[0].start_time < first_event_time
        ):
            first_event_time = connection.transactions[0].queries[0].start_time
        if (
            connection.transactions[-1].queries[-1].end_time
            and connection.transactions[-1].queries[-1].end_time > last_event_time
        ):
        ...
        with open("replayerrors000", "w", newline="") as output_file:
            try:
                dict_writer = csv.DictWriter(
                    output_file, fieldnames=error_list[0].keys()
                )
