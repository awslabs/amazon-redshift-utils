...
(
  ...
  FROM (SELECT ROW_NUMBER() OVER () AS n
        FROM svl_query_report LIMIT 604800)
),
...
(
  SELECT service_class,
         MAX(queue_end_time) max_queue_end_time
  FROM stl_wlm_query
  WHERE total_queue_time > 0
  GROUP BY service_class
)

SELECT apex.service_class,
       apex.num_query_tasks AS max_wlm_concurrency,
       apex.service_class_slots AS max_service_class_slots,
       MAX(apex.dt) max_slots_ts,
       queued.max_queue_end_time last_queued_time
FROM apex
  JOIN maxes
...
GROUP BY apex.service_class,
         ...
         apex.service_class_slots,
         queued.max_queue_end_time
