...
with revenue as (
		...
		where
			...
			and l_shipdate < dateadd(month, 3, cast('1994-06-01' as date)) 
		group by
			l_suppkey
)
select /* TPC-H Q15 */
...
from
	lineitem
where
	l_shipdate <= dateadd(day, -79, cast('1998-12-01' as date)) 
group by
	l_returnflag,
	l_linestatus
order by
...
where
	...
	and o_orderdate < dateadd(month, 3, cast('1994-10-01' as date))
	and l_returnflag = 'R'
...
where
	...
	and o_orderdate < dateadd(month, 3, cast('1994-08-01' as date)) 
	and exists (
		select
			*
		from
