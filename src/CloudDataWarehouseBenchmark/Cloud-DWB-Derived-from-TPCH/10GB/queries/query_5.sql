...
with revenue as (
		...
		where
			...
			and l_shipdate < dateadd(month, 3, cast('1996-02-01' as date)) 
		group by
			l_suppkey
)
select /* TPC-H Q15 */
...
where
	...
	and o_orderdate < dateadd(month, 3, cast('1996-03-01' as date)) 
	and exists (
		select
			*
		from
...
where
	...
	and l_shipdate < dateadd(month, 1, cast('1994-05-01' as date))
;
...
from
	lineitem
where
	l_shipdate <= dateadd(day, -100, cast('1998-12-01' as date)) 
group by
	l_returnflag,
	l_linestatus
order by
...
where
	...
	and o_orderdate < dateadd(month, 3, cast('1994-12-01' as date))
	and l_returnflag = 'R'
