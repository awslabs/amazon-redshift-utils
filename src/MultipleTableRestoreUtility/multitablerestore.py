...
class rsrestore:
    ...
    def __init__(self,clusterIdentifier,snapshotIdentifier,sourceDatabaseName,sourceSchemaName,targetDatabaseName,targetSchemaName):
    ...
    def restoretable(self,srcTable,tgtTable):
        try:
            ...
            SourceDatabaseName = self.srcDatabase,SourceSchemaName = self.srcSchema,SourceTableName = srcTable,TargetDatabaseName = self.tgtDatabase,
    ...
    def printMessage(self):
        if self.restorestatus('Status') == 'SUCCEEDED':
            print "\nDETAIL: Table %s.%s restored to database %s. Total size restored is %sMB." % (self.restorestatus('TargetSchemaName'),self.restorestatus('NewTableName'),self.restorestatus('TargetDatabaseName'),self.restorestatus('TotalDataInMegaBytes'))
        else:
...
def main(input_args):
    ...
    if (tgtdbname == None) or (srcdbname == None) or (snapshotid == None) or (clusterid == None) or (filename == None):
