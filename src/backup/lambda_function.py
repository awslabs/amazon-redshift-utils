...
def event_handler(event, context):
    ...
    #outputFileName = configDetail["outputFile"] + timestr +".sql"
    #print outputFileName 
    # run the column encoding utility, if requested
    if "ColumnEncodingUtility" in config["utilities"]:
	#                                          _output_file,                       _db,                _db_user,      _db_pwd,               _db_host,               _db_port,               _analyze_schema,               _target_schema,              _analyze_table, _analyze_col_width,                     _threads,                                _do_execute,            _query_slot_count,                 _ignore_errors,                 _force,               _drop_old_data,                _comprows,              _query_group,               _debug, _ssl_option
